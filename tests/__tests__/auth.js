...
describe("Sign up", () => {
  test("Sign up validation sends back errors if not all fields are filled", async (done) => {
    ...
    expect(
      res.body.errors.filter((error) =>
        error.msg.match(/username must be specified/i)
      ).length
    ).toBe(1);
    expect(
      res.body.errors.filter((error) =>
        error.msg.match(/email must be specified/i)
      ).length
    ).toBe(1);
    expect(
      res.body.errors.filter((error) =>
        error.msg.match(/password must be specified/i)
      ).length
    ).toBe(1);
    done();
  });
